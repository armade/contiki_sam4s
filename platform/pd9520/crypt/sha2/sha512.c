...
void sha2_sha512_process( SHA512_CTX *ctx, const unsigned char data[128] )
{
    int i;
...
void sha2_sha512_update( SHA512_CTX *ctx, const uint8_t data[], size_t len)
{
    ...
    if( left && len >= fill )
    {
        ...
        sha2_sha512_process( ctx, ctx->buffer );
        data += fill;
        len  -= fill;
        left = 0;
    }
...
{
	sha2_sha512_update( (SHA512_CTX *)ctx, data, len);
}
...
void sha2_sha512_final( SHA512_CTX *ctx, uint8_t hash[]  )
{
    ...
    sha2_sha512_update( ctx, sha512_padding, padn );
    sha2_sha512_update( ctx, msglen, 16 );
...
void sha2_sha384_final( SHA384_CTX *ctx, uint8_t hash[] )
{
	...
	sha2_sha384_update( ctx, sha512_padding, padn );
	sha2_sha384_update( ctx, msglen, 16 );
             ...
             unsigned char hash[])
{
    ...
    sha2_sha512_update( &ctx, input, len );
    sha2_sha512_final( &ctx, hash );
}
             ...
             unsigned char hash[])
{
    ...
    sha2_sha384_update( &ctx, input, len );
    sha2_sha384_final( &ctx, hash );
}
