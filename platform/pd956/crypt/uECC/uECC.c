...
static void bcopy_uECC(uint8_t *dst,
                  const uint8_t *src,
                  unsigned num_bytes) {
    while (0 != num_bytes) {
        num_bytes--;
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy_uECC((uint8_t *) _private, private_key, num_bytes);
    bcopy_uECC((uint8_t *) _public, public_key, num_bytes*2);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy_uECC((uint8_t *) secret, (uint8_t *) _public, num_bytes);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy_uECC(public_key, compressed+1, curve->num_bytes);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy_uECC((uint8_t *) native, bits, bits_size);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy_uECC((uint8_t *) tmp, private_key, BITS_TO_BYTES(curve->num_n_bits));
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy_uECC((uint8_t *) signature + curve->num_bytes, (uint8_t *) s, curve->num_bytes);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy_uECC((uint8_t *) r, signature, curve->num_bytes);
    bcopy_uECC((uint8_t *) s, signature + curve->num_bytes, curve->num_bytes);
#else
    uECC_vli_bytesToNative(_public, public_key, curve->num_bytes);
    uECC_vli_bytesToNative(
